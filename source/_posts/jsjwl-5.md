...
应用层是计算机网络体系结构的最顶层，是设计和建立计算机网络的最终目的，也是计算机网络中发展最快的部分。早期基于文本的应用（电子邮件、远程登陆、文件传输、新闻组）。20世纪90年代将因特网带入千家万户的万维网www。当今流行的即时通讯、p2p文件共享以及各种音视频应用。计算机设备的小型化和“无处不在”，宽带住宅接入和无线接入的日益普及和迅速发展，为未来更多的新型应用提供了广阔的舞台。
## 域名系统 DNS
